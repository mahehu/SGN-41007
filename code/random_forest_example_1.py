...
if __name__ == "__main__":
    ...
    plt.savefig("../images/RandomForest1.pdf", transparent = True, bbox_inches = "tight")
#
#    for k, estimator in enumerate(clf.estimators_):
#        dotName = "tree_graph_%i.dot" % k
#        with open(dotName, 'w') as f:
#            f = tree.export_graphviz(estimator, out_file=f, label = 'all', class_names = ['BLUE', 'RED'])
#        
#        command = "dot -Tpdf %s -o %s" % (dotName, dotName[:-4] + ".pdf")
#        os.system(command)
#        os.unlink(dotName)    
#    
#        visualize(estimator, X, y, None, zoom = [-0.5,1.5,-4,-2])
#        plt.savefig("tree_%i.pdf" % k, bbox_inches = "tight")